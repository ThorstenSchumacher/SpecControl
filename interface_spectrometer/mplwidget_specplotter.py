...
class MplWidget(QWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.xax = np.linspace(0, 1919, 1920)    # this is and will ever be the pixel axis of our sensor
        self.yax = np.zeros((1920, 1))            # this is our intensity spectrum from the sensor
        ...
        self.ylamp = np.ones((1,1920))           # we start with a flat equal 1 lamp spectrum
        self.yback = np.zeros((1,1920))          # we start with a flat equal 0 background spectrum
        ...
        # plot x range (ids)
        self.plotxrange = [0, 1919]  # min value and max value
        self.plotyrange = [0, 1]

        # zoombox ranges
        self.zoomboxX = [0, 1919]
        self.zoomboxY = [0, 1]
        ...
        self.line1, = self.canvas.axes.plot(self.xax, self.yax, color =(100/255, 150/255, 200/255))
        self.line2, = self.canvas.axes.plot([self.xid, self.xid], [0, 1e8], color =(200/255, 200/255, 200/255))
        self.line3, = self.canvas.axes.plot([0, 1e8], [self.yid, self.yid], color =(200/255, 200/255, 200/255))
...
########################################### spectrum update functions #################################################
    ...
    def showlivedata(self):
        self.anim = animation.FuncAnimation(self.canvas.figure, self.data_update, init_func=self.ani_init, interval=1, blit=False, save_count=1)
        plt.show()
